...
class JvmFlagsTunerInterface(opentuner.measurement.MeasurementInterface):
	...
	if not os.path.exists(self.tuner_config_file_folder):
	    os.makedirs(self.tuner_config_file_folder)	
    ...
    def read_flag_dependancy_structure(self):
		...
		print ('flagsfolder' + self.flags_folder)
            ...
            elif flag_type == 'douauto':
                self.douauto=True
                self.douauto_bool,self.douauto_param = self.prepare_flags(self.flags_folder+'DOUauto/douauto.csv')
                self.bool_flags.append(self.douauto_bool)
                self.param_flags.append(self.douauto_param)
    ...
    def run(self, desired_result, input, limit):
        ...
        if(self.douauto):
            for flag in self.douauto_bool:
                self.add_to_flags_bool(cfg, flag)
            for flag in self.douauto_param:
                self.add_to_flags_param(cfg, flag,self.douauto_param )
        global count
        count = count +1
        #print (count)
